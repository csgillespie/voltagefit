...
#'
#' Plot curves, given by \code{\link{calc_curves}}, for each week provided.
#'
#' @inheritParams   fit_manova
#' @inheritParams   plot_under
#' @param wcurves   Week curves as given by \code{\link{calc_curves}}.
#' @param fm        Fitted manova as given by \code{\link{fit_manova}}.
#' 
...
#' plot_week = function(fitted, wcurves, fitman)
#'
#' @export
plot_week = function(fitted, wcurves, fm, orig=TRUE){
  v = attr(fitted,"v")
...
#'
#' Plot underlying curves, given by \code{\link{calc_curves}}.
#' N.B. using the mean with low levels of data can lead to spurious curves, 
#' the median appears to be more stable!
#' 
#' @inheritParams   fit_manova
#' @param underc    Underlying curve as given by \code{\link{calc_curves}}.
...
#' underc = calc_curves(fitted, unders)
#' plot_under(fitted, underc)
#' 
#' @importFrom matrixStats colQuantiles
#'
#' @export
plot_under = function(fitted, underc, orig=T, med=F){
  v = attr(fitted,"v")
  tot_f = which(diff(v) < 0)[1]
  v_f = v[1:tot_f]
  v_b = v[tot_f:length(v)]
