...
fitmanova = function(fitall,design)
{
  ...
  man_w_f = manova(cbind(X1,X2,X3,X4,X5,X6) ~ factor(week)+factor(name), data=fdata,weights=1/fitall$forward$cost$cost) 
  m_f = lm(cbind(X1,X2,X3,X4,X5,X6) ~ factor(week)+factor(name), data=fdata)

  man_w_b = manova(cbind(X1,X2,X3,X4,X5,X6) ~ factor(week)+factor(name), data=bdata,weights=1/fitall$forward$cost$cost) 
