...
plotweek = function(wafer,curves,orig=T,tot_obs=241,tot_f=193)
{
  ...
  if(orig==F)
  {
    ...
    plot(v_f,curves$forward[1,],type="l",xlab="Vg (V)",ylab="Id (A)",log="y"
         ,ylim=c(0.3,1.0),main="Transformed scale")
    grid()
    ...
    temp=numeric(length(unique(wafer$test_date)))
    for(i in 1:length(unique(wafer$test_date)))
    {
      temp[i] = paste("Week ",i,sep="")
    }
    
    legend("topleft",temp,col=c(1:length(unique(wafer$test_date))),lty=rep(1,length(unique(wafer$test_date))))
    legend("bottomright",c("Forwards","Backwards"),col=c(1,1),lty=c(1,2))
  }
  else
  {
    ...
    par(mar=c(3,3,2,1), mgp=c(2,0.4,0), tck=-.01,cex.axis=0.9, las=1)
    plot(v_f,exp(m/curves$forward[1,]),type="l",xlab="Vg (V)",ylab="Id (A)",log="y"
         ,ylim=c(2e-11,1e-03),main="")
    grid()
    
    for(i in 2:nrow(curves$forward))
    {
      lines(v_f,exp(m/curves$forward[i,]),col=i)
    }
    
    for(i in 1:nrow(curves$forward))
    {
      lines(v_b,exp(m/curves$backward[i,]),col=i,lty=2)
    }
