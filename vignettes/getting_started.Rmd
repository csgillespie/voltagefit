...
You can plot the parameter values and cost function via

```{r, fig.keep="first", fig.width=6, fig.height=4}
...
Calculate the week parameters (the parameters for each curve corresponding to each week)
```{r}
weekp = week_param(fit_man)
```
Calculate the underlying parameters (the parameters for the underlying curve)
```{r}
underp = under_param(fit_man)
```
