...
##HERE BE DRAGONS...
The voltage x values in the forward curves are unevenly distributed, so least squares emphasises the fit in high density areas.. 
<!--This method uses interpolation to make the x points evenly distributed before fitting, making the fit "look" better in the high voltage tail.
...
###IDEA: Least Area fitting.
Instead of minimising the squares of the residuals, how about trying to minimise the area.

Write down an integral of the difference between two linear lines. One line going through the data points, one going through the fitted points.

This integral gives the area between the two curves. Now discretise the data and fitted curves using linear interpolation. This results in an integral that can be approximated similarly to using the trapezium rule. The area between point \(x[n]\) and \(x[n+1]\) is then,

$$A_n = \frac{1}{2} (x_{n+1} - x_{n}) (y_n-z_n+y_{n+1}-z_{n+1}),$$

where $y_n$ is the data points and $z_n$ takes the value of the fitted curve for all $x_n$. The cost can then be defined as,
